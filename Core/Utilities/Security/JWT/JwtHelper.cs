...
namespace Core.Utilities.Security.JWT
{
    public class JwtHelper : ITokenHelper
    {
        ...
        public AccessToken CreateToken(User user, List<OperationClaim> operatingClaims)
        {
            _accessTokenExpiration = DateTime.Now.AddMinutes(_tokenOptions.AccessTokenExpiration);
            var securityKey = SecurityKeyHelper.CreateSecurityKey(_tokenOptions.SecurityKey);
            var signingCredentails = SigningCredentialsHelper.CreateSigningCredentials(securityKey);
            var jwt = CreateJwtSecurityToken(_tokenOptions, user, signingCredentails, operatingClaims);
        ...
        private IEnumerable<Claim> SetClaims(User user, List<OperationClaim> operationClaims)
        {
            ...
            claims.AddNameIdentify(user.Id.ToString());
